...
public:
    ...
    const sensor_msgs::JointState::ConstPtr & joint_state_msg)
  {
    ...
    traj.joint_names.push_back("wrist_3_joint");
     ...
     std::vector<double> goal = {1.76, 2.1,-0.70, 1.5, 3.27, -1.51, 0.0};
    if(num_tools_in_tray_ == 0 && order_0_ == false){
      ...
      if(gripper_state_attatch_ == false) {
        pass_order0[1] = 0;
        if(order_0_ever_attatched == false)
        move_to(p1,p1,traj,joint_state_msg,pass_order0[0]);
        else
        move_to(middle_p,p2,traj,joint_state_msg,pass_order0[0]);
      }
      else {
              pass_order0[0] = 0;
              order_0_ever_attatched = true;
              move_to(middle_p,goal,traj,joint_state_msg,order_0_,pass_order0[1]);
      }

      if(num_tools_in_tray_ == order_num_ + 1) {
        order_0_ = true;
        order_num_++;
      }
    }
    ...
    if(num_tools_in_tray_ == 2 && order_2_ == false){
      grasp_bin8();
      std::vector<double> middle_p = {1.35, 0.46, -1.0, 2.00, 3.58,-1.51,0.0};
    ...
    if(num_tools_in_tray_ == 3 && order_3_ == false){
      grasp_bin8();
      std::vector<double> middle_p = {1.35, 0.5, -1.1, 2.0, 3.58,-1.51,0.0};
    ...
    if(num_tools_in_tray_ == 4 && order_4_ == false){
      grasp_bin8();
      std::vector<double> middle_p = {1.35, 0.50, -1.0, 2.0, 3.58,-1.51,0.0};
  ...
  void send_arm_to_zero_state() {
    ...
    msg.points[0].positions = {1.76, 0.0, -1.0, 3.14,3.58,-1.51,0.0};
    ...
    /*
      ...
      void grasp_bin8() {
        ...
        // Listen to tf
          ...
          geometry_msgs::Point relative_pose_bin8;	///< the relative position between gripper and bin
        tf::StampedTransform transform_bin8;		// initialize transform between gripper and bin8
        listener.waitForTransform("/bin8_frame","/vacuum_gripper_link",ros::Time(0), ros::Duration(10.0));
          listener.lookupTransform("/bin8_frame","/vacuum_gripper_link",ros::Time(0),transform_bin8);
        relative_pose_bin8.x = transform_bin8.getOrigin().x();
        relative_pose_bin8.y = transform_bin8.getOrigin().y();
        relative_pose_bin8.z = transform_bin8.getOrigin().z();

        //ROS_INFO_STREAM("relative pose to bin6: x = " << relative_pose_bin8.x);
       // ROS_INFO_STREAM("relative pose to bin6: y = " << relative_pose_bin8.y);
        //ROS_INFO_STREAM("relative pose to bin6: z = " << relative_pose_bin8.z);
        ...
        if (under_tolerance(tolerance, relative_pose_bin8)) {
          grasp_kit();
        }
    }
      /*
